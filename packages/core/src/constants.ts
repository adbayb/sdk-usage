	...
	return (
		<Box width={400}>
			<TextField
				...
				InputProps={{
					startAdornment: (
						<Show when={select.hasValue()}>
							<Show
								when={
									!select.multiple && !select.hasInputValue()
								}
							>
								<Box
									position="absolute"
				...
				placeholder={
					!select.multiple && select.hasValue()
						? ""
						: props.placeholder
				}
			/>
						...
						>
							...
							>
								{(option) => (
									<ListItemButton
										onClick={() =>
											select.pickOption(option)
										}
										selected={select.isOptionFocused(
											option
										)}
										disabled={select.isOptionDisabled(
											option
										)}
									>
										<ListItemText
